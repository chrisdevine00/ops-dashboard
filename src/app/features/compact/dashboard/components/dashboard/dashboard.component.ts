...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  siteTimeShort = '';
  localTimeShort = '';
  private readonly siteTimezone = 'Europe/London';
  ...
  private loadSystemData(): void {
    this.systemsService.getSystemBySerialNumber(this.serialNumber)
  ...
  private generateMockWorkflows(count: number, laneCount: number, allowedTypes?: string[]): WorkflowTask[] {
    ...
    for (const item of startTimes) {
      ...
      const status: 'normal' | 'warning' | 'alert' =
        durationHours >= 12 ? 'alert' :
        durationHours >= 4 ? 'warning' :
        'normal';
  ...
  private createGanttChart(title: string, workflows: WorkflowTask[], laneCount: number, customLabels?: string[], showXAxis: boolean = true): EChartsOption {
    ...
    workflows.forEach((workflow) => {
      ...
      const color = workflow.status === 'alert' ? BD_AURORA_LIGHT :
                    workflow.status === 'warning' ? BD_AURORA :
                    BD_COMET;
    ...
    const workflows: DeviceWorkflow[] = [
      {
        ...
        states: [
          ...
          { timestamp: new Date(baseDate.getTime() + 30 * 60000 + 30000), state: 'Powered on Starting' },
          { timestamp: new Date(baseDate.getTime() + 30 * 60000 + 45000), state: 'Offline Idle' },
          { timestamp: new Date(baseDate.getTime() + 30 * 60000 + 60000), state: 'Offline Starting' },
          { timestamp: new Date(baseDate.getTime() + 30 * 60000 + 90000), state: 'Online Pause Idle' },
          { timestamp: new Date(baseDate.getTime() + 30 * 60000 + 120000), state: 'Online' },
        ]
      },
    ];
  ...
  private generateMockPxAlerts(): { timestamp: Date; severity: 'warning' | 'error' | 'info'; source: string; message: string; code: string }[] {
    ...
    this.gxAlerts = [
      { minutesFromStart: 120, severity: 'warning' as const, source: 'Left GX', message: 'HPV workflow delayed' },
      { minutesFromStart: 360, severity: 'error' as const, source: 'Left GX', message: 'CBC analysis failed' },
    ].map(a => ({
    ...
    this.mxAlerts = [
      { minutesFromStart: 180, severity: 'error' as const, source: 'Right MX', message: 'FluA processing error' },
      { minutesFromStart: 540, severity: 'warning' as const, source: 'Right MX', message: 'VP workflow warning' },
    ].map(a => ({
      timestamp: new Date(baseDate.getTime() + a.minutesFromStart * 60000),
      severity: a.severity,
      source: a.source,
  ...
  private createStateTransitionChart(): EChartsOption {
    ...
    const alertData = alerts.map(alert => {
      ...
      const color = alert.severity === 'error' ? BD_INFRARED :
                    alert.severity === 'warning' ? BD_AURORA : '#333333';
      return {
