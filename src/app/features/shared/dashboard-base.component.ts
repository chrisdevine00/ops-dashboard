...
import { SwimLaneConfigService } from '../../core/services/swim-lane-config.service';
import { SwimLaneConfig } from '../../core/models/swim-lane.model';
...
export abstract class DashboardBaseComponent implements OnInit, OnDestroy {
  // System info from route
  serialNumber = '';
  facilityName = '';
  system: System | undefined;

  // Swim lane configuration
  private swimLaneConfigService = inject(SwimLaneConfigService);
  swimLanes: SwimLaneConfig[] = [];
  laneCharts: LaneChart[] = [];

  // Layout options (subclasses can override)
  protected showAllXAxes = true;

  // PX System Events
  protected pxSystemEvents: {
    hour: number;
  ...
  siteTimeShort = '';
  localTimeShort = '';
  protected readonly siteTimezone = 'Europe/London';
  ...
  // Auto-refresh
  private readonly REFRESH_INTERVAL = 10 * 60;
  ...
  constructor(
    ...
    protected systemsService: SystemsService
  ) {}
  ...
  private loadSystemData(): void {
    this.systemsService
      .getSystemBySerialNumber(this.serialNumber)
      .pipe(takeUntil(this.destroy$))
      .subscribe(system => {
        this.system = system;
        this.facilityName = system?.customerName || 'Unknown System';
        if (system?.moduleConfiguration) {
          this.swimLanes = this.swimLaneConfigService.buildSwimLanes(system.moduleConfiguration);
        }
        this.generateCharts();
      });
  }
  ...
  private generateMockWorkflows(
    ...
    const testStartOffset = 45 * 60000;
    ...
    for (const item of startTimes) {
      ...
      if (lane === -1) {
        ...
        item.time = laneEndTimes[lane] + 5 * 60000;
      }
      ...
      const status: 'normal' | 'warning' | 'alert' =
        durationHours >= 12 ? 'alert' : durationHours >= 4 ? 'warning' : 'normal';
  ...
  private createGanttChart(
    ...
    // Analyzer state transition data
    const STATE_TRANSITION_Y = 3;
    const analyzerStateData: any[] = [];
    const analyzerEventLines: { hour: number; type: 'boot' | 'shutdown' }[] = [];

    if (analyzerStates && analyzerStates.length > 0) {
      // One dot per workflow at its start time (mirrors PX Device State pattern)
      analyzerStates.forEach((workflow, index) => {
        const hoursFromStart =
          (workflow.startTime.getTime() - baseDate.getTime()) / (1000 * 60 * 60);
        analyzerStateData.push({
          value: [hoursFromStart, STATE_TRANSITION_Y],
          workflowIndex: index,
          workflow: workflow,
        });
      });

      // Boot line at the workflow start time
      for (const workflow of analyzerStates) {
        const hour = (workflow.startTime.getTime() - baseDate.getTime()) / (1000 * 60 * 60);
        analyzerEventLines.push({ hour, type: 'boot' });
      }
    }
    ...
    return {
        ...
        formatter: (params: any) => {
          if (params.componentType === 'series' && params.data.workflowData) {
            ...
            const duration = Math.round(
              (wf.end.getTime() - wf.start.getTime()) / (1000 * 60)
            );
            return `
          ...
          if (params.seriesName === 'Analyzer States' && params.data.workflow) {
            const workflow = params.data.workflow as DeviceWorkflow;
            let html =
              '<div style="min-width: 250px;"><strong>Analyzer State Workflow</strong><br/><br/>';
            html += '<table style="width: 100%; border-collapse: collapse;">';
            html +=
              '<tr style="border-bottom: 1px solid rgba(255,255,255,0.2);"><th style="text-align: left; padding: 2px 8px 2px 0;">Time</th><th style="text-align: left; padding: 2px 0;">State</th></tr>';
            workflow.states.forEach(state => {
              const timeStr = state.timestamp.toLocaleTimeString('en-US', {
                hour: '2-digit',
                minute: '2-digit',
                second: '2-digit',
                hour12: false,
      ...
      series: [
        ...
        },
        {
          ...
          type: 'custom',
          renderItem: (params: any, api: any) => {
            const y = api.coord([0, ALERTS_Y_INDEX - 0.5]);
            const xStart = api.coord([0, 0]);
            const xEnd = api.coord([24, 0]);
        ...
        },
        {
          ...
          type: 'custom',
          renderItem: (params: any, api: any) => {
            const midnightX = api.coord([17, 0]);
            const yTop = api.coord([0, ALERTS_Y_INDEX]);
            const yBottom = api.coord([0, 0]);
            const chartHeight = yBottom[1] - yTop[1] + api.size([0, 1])[1];
        ...
        {
          name: 'Analyzer States',
          type: 'scatter',
          data: analyzerStateData,
          symbolSize: 12,
          itemStyle: {
        ...
        {
          name: 'Analyzer State Lines',
          type: 'custom',
          renderItem: (params: any, api: any) => {
            const event = analyzerEventLines[params.dataIndex];
            if (!event) return;
          ...
          data: analyzerEventLines.map((e, i) => ({ value: i, event: e })),
          tooltip: {
            formatter: (params: any) => {
              const event = analyzerEventLines[params.dataIndex];
              if (!event) return '';
              ...
              const minutes = Math.round((event.hour % 1) * 60);
              const timeStr = `${Math.floor(hour).toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')}`;
              const label = event.type === 'boot' ? 'Analyzer Boot' : 'Analyzer Shutdown';
              return `<strong>${label}</strong><br/>Time: ${timeStr}`;
            },
          },
          z: 1,
        },
  ...
  protected generateCharts(): void {
    this.pxSystemEvents = [
      { hour: 0, type: 'powerCycle', label: 'Power Cycle' },
      { hour: 0.25, type: 'boot', label: 'System Boot' },
    ];

    this.laneCharts = [];

    for (const lane of this.swimLanes) {
      const label = this.swimLaneConfigService.getLaneLabel(lane);

      if (lane.position === 'px') {
        this.laneCharts.push({
          config: lane,
          label,
          serial: this.serialNumber,
          options: this.createStateTransitionChart(),
        });
      } else {
        // Analyzer lane (MX or GX)
        const workflowLaneCount = 3;
        const yAxisLabels = [
          'Workflow 1', 'Workflow 2', 'Workflow 3',
          'State Transition',
          'Alerts & Errors',
        ];
        const allowedTypes = lane.moduleName === 'GX'
          ? ['HPV']
          : ['CBC', 'CHEM', 'UA', 'LIP', 'TSH', 'LFT', 'REN', 'GLU', 'A1C', 'ELEC', 'COAG'];
        const analyzerStates = this.generateMockAnalyzerStates();
        const workflows = this.generateMockWorkflows(20, workflowLaneCount, allowedTypes);

        this.laneCharts.push({
          config: lane,
          label,
          serial: lane.moduleSerialNumber,
          options: this.createGanttChart(
            label,
            workflows,
            workflowLaneCount,
            yAxisLabels,
  ...
  private generateMockDeviceWorkflows(): DeviceWorkflow[] {
    ...
    const workflows: DeviceWorkflow[] = [
      {
        startTime: new Date(baseDate.getTime() + 30 * 60000),
        states: [
          { timestamp: new Date(baseDate.getTime() + 30 * 60000), state: 'Powered Off' },
          {
            timestamp: new Date(baseDate.getTime() + 30 * 60000 + 30000),
            state: 'Powered on Starting',
          },
          {
            timestamp: new Date(baseDate.getTime() + 30 * 60000 + 45000),
            state: 'Offline Idle',
          },
          {
            timestamp: new Date(baseDate.getTime() + 30 * 60000 + 60000),
            state: 'Offline Starting',
          },
          {
            timestamp: new Date(baseDate.getTime() + 30 * 60000 + 90000),
            state: 'Online Pause Idle',
          },
          {
            timestamp: new Date(baseDate.getTime() + 30 * 60000 + 120000),
            state: 'Online',
          },
        ],
      },
  ...
  private generateMockAnalyzerStates(): DeviceWorkflow[] {
    const baseDate = new Date(this.currentDate);
    baseDate.setHours(7, 0, 0, 0);

    // Analyzer boot mirrors PX boot (~7:30 AM), offset slightly after PX
    const bootMs = baseDate.getTime() + 32 * 60000; // 7:32 AM

    return [{
      startTime: new Date(bootMs),
      states: [
        { timestamp: new Date(bootMs), state: 'Powered Off' },
        { timestamp: new Date(bootMs + 15000), state: 'Powered On Starting' },
        { timestamp: new Date(bootMs + 30000), state: 'Offline Idle' },
        { timestamp: new Date(bootMs + 50000), state: 'Offline Starting' },
        { timestamp: new Date(bootMs + 80000), state: 'Online Pause Idle' },
        { timestamp: new Date(bootMs + 120000), state: 'Online' },
      ],
    }];
  }
    ...
    this.gxAlerts = [
      {
        ...
        source: 'Left GX',
        message: 'HPV workflow delayed',
      },
      {
        minutesFromStart: 360,
        severity: 'error' as const,
        source: 'Left GX',
        message: 'CBC analysis failed',
      },
    ].map(a => ({
    ...
    this.mxAlerts = [
      {
        ...
        severity: 'error' as const,
        source: 'Right MX',
        message: 'FluA processing error',
      },
      {
        minutesFromStart: 540,
        severity: 'warning' as const,
        source: 'Right MX',
        message: 'VP workflow warning',
      },
    ].map(a => ({
      timestamp: new Date(baseDate.getTime() + a.minutesFromStart * 60000),
      severity: a.severity,
  ...
  private createStateTransitionChart(): EChartsOption {
    ...
    const alertData = alerts.map(alert => {
      const hoursFromStart =
        (alert.timestamp.getTime() - baseDate.getTime()) / (1000 * 60 * 60);
      const color =
      ...
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          if (params.seriesName === 'Device State' && params.data.workflow) {
            const workflow = params.data.workflow as DeviceWorkflow;
            let html =
              '<div style="min-width: 250px;"><strong>Device State Workflow</strong><br/><br/>';
            html += '<table style="width: 100%; border-collapse: collapse;">';
            html +=
              '<tr style="border-bottom: 1px solid rgba(255,255,255,0.2);"><th style="text-align: left; padding: 2px 8px 2px 0;">Time</th><th style="text-align: left; padding: 2px 0;">State</th></tr>';
